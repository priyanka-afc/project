...
class NaivebayesDPH:
    ...
    def extract_features(self,tweets, batch_size=500, test_file=True, feat_type='presence'):
    ...
    def mainmethod(self):
        ...
        #if USE_BIGRAMS:
            #bigrams = utils.top_n_bigrams(self.BI_FREQ_DIST_FILE, self.BIGRAM_SIZE)
        tweets = self.process_tweets(self.TRAIN_PROCESSED_FILE, test_file=False)
        if self.TRAIN:
        ...
        for training_set_X, training_set_y in self.extract_features(train_tweets, test_file=False, feat_type=self.FEAT_TYPE,
        ...
        if self.TRAIN:
            ...
            for val_set_X, val_set_y in self.extract_features(val_tweets, test_file=False, feat_type=self.FEAT_TYPE,
                                                         batch_size=batch_size):
                if self.FEAT_TYPE == 'frequency':
            ...
            del train_tweets
            test_tweets = self.process_tweets(self.TEST_PROCESSED_FILE, test_file=True)
            n_test_batches = int(np.ceil(len(test_tweets) / float(batch_size)))
            predictions = np.array([])
            print ('Predicting batches')
            i = 1
            for test_set_X, _ in self.extract_features(test_tweets, test_file=True, feat_type=self.FEAT_TYPE):
